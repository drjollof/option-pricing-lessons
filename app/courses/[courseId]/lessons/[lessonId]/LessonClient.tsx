...
export const LessonClient: React.FC<{ lesson: Lesson, courseId: string }> = ({ lesson, courseId }) => {
  ...
  }, [activePhaseIndex, lesson.id, lesson.phases.length, markComplete, courseId]);
    ...
    <main className="min-h-screen bg-[#f8fafc] dark:bg-slate-950 p-6 md:p-10 font-sans selection:bg-blue-200 transition-colors">
      <div className="max-w-7xl mx-auto">
        ...
        <div 
          ...
          className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-200/60 dark:border-slate-800/60 p-6 md:p-8 backdrop-blur-xl mb-12 scroll-mt-6"
        >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 md:px-6 shadow-sm gap-4 mb-8">
          <button 
            disabled={activePhaseIndex === 0} 
            onClick={() => setActivePhaseIndex(i => i - 1)}
            className="w-full sm:w-auto px-5 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors flex items-center justify-center gap-2"
          ...
          <button 
            ...
            className="text-sm font-bold text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 transition-colors uppercase tracking-widest flex items-center gap-2"
          >
            Back to Top
